...
const ProductDetails = () => {
  ...
  // ✅ Correct user selector (single object)
  const user = useSelector((state) => state.user.user);

  useEffect(() => {
    dispatch(getCurrentUser()); // load logged-in user from localStorage
  }, [dispatch]);
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      await dispatch(asyncUpdateProduct(id, editData));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white py-12 flex flex-col items-center">
      ...
      <div className="w-full max-w-6xl flex flex-col md:flex-row gap-10 px-4 md:px-0">
        ...
        {/* ✅ Admin Only Form (based on isAdmin flag) */}
        {user?.isAdmin && (
          <div className="flex-1 bg-white shadow-2xl rounded-2xl p-8 border border-gray-100 overflow-y-auto max-h-[90vh] relative">
            ...
            <p className="text-center text-gray-500 text-sm mb-6">
              Make changes and update product info in{" "}
              <span className="text-blue-600 font-semibold">EcomX</span>
            </p>
            ...
            <form onSubmit={handleUpdate} className="space-y-4">
              ...
              <button
                ...
                className="w-full mt-2 bg-red-600 hover:bg-red-700 text-white py-2.5 rounded-lg font-semibold text-sm tracking-wide transition-all duration-200 shadow-md"
              >
                🗑️ Delete Product
              </button>
            </form>
